...
namespace KernelV6
{
	...
	__global__ void HBFSearchV6Atomic64(
		...
		int2* devBF, int*devBFSize,int* devF2,
		...
		int level)
	{
		...
		for (int i = tileID; i < (devBFSize[0] + VW_SIZE - 1) / VW_SIZE; i += IDStride)
		{
			int j = i * VW_SIZE + tile.thread_rank();
			int2 node2 = devBF[j];
			int node = node2.x;
			int k = node2.y;
			int sourceWeight = devDistances[node].y;
			int nodeS = devNodes[node];
			relaxEdges += 1;
			int flag = 0;
			int2 dest = devEdges[nodeS + k];
...
#define kernelV6Atmoic64(vwSize,gridDim, blockDim,sharedLimit) \
HBFSearchV6Atomic64<vwSize><<<gridDim, blockDim, sharedLimit>>> \
(devUpOutNodes, devUpOutEdges, devInt2Distances, devBF,devBFSize, devF2, devSizes, sharedLimit, level)

#define switchKernelV6(atomic64,gridDim, blockDim,  sharedLimit ) \
{\
	if (atomic64) {  \
		kernelV6Atmoic64(WARPSIZE,gridDim, blockDim, sharedLimit); \
	} \
	else { \
		__ERROR("no atomic32") \
	}\
...
#define switchKernelV6Config(configs) \
	switchKernelV6(configs.atomic64,gdim, bdim,sharedLimit)