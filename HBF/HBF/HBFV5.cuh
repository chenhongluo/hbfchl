...
namespace KernelV5
{
	...
	__global__ void DynamicParrallel(
		int2 *__restrict__ devEdges,
		int2 *__restrict__ devDistances,
		int sourceWeight,int nodeS,int nodeE,
		int* devF2,
		int *__restrict__ devSizes,
		int level)
	{
		// printf("%d,%d,%d\n",nodeS,nodeE,blockDim.x * gridDim.x);
		int realID = blockIdx.x * gridDim.x + threadIdx.x;
		for (int k = nodeS + realID; k < nodeE + realID;k += blockDim.x * gridDim.x)
		   {
			   ...
			   if (k < nodeE)
			   {
				   ...
				   if(flag){
						int pos = atomicAdd(devSizes+1,1);
						devF2[pos] = dest.x;
					}
				}
		   }
		...
		int level)
	{
		...
		for (int i = tileID; i < kkk; i += IDStride)
		{
			...
			if(i < devSizes[2]){
				...
				if(relaxSize>dp){
					if(tileThreadId == 0){
						// DynamicParrallel<32><<<1,1024>>>(devEdges,devDistances,
						// 	sourceWeight,nodeS,nodeE,devF2,devSizes,level);
					}
					relaxSize = 0;
					nodeE = nodeS;
				}
...
#define kernelV5Atmoic64(vwSize,gridDim, blockDim,sharedLimit) \
HBFSearchV5Atomic64<vwSize> << <gridDim, blockDim, sharedLimit >> > \
(devUpOutNodes, devUpOutEdges, devInt2Distances, devF3, devF2, devSizes, sharedLimit,dp, level)

#define switchKernelV5(atomic64,vwSize,gridDim, blockDim,  sharedLimit ) \
{\
	if (atomic64) {  \
		switch (vwSize) { \
		case 1:\
			kernelV5Atmoic64(1,gridDim, blockDim, sharedLimit);break; \
		case 2: \
			kernelV5Atmoic64(2,gridDim, blockDim, sharedLimit); break;\
		case 4: \
			kernelV5Atmoic64(4,gridDim, blockDim, sharedLimit); break;\
		case 8: \
			kernelV5Atmoic64(8,gridDim, blockDim, sharedLimit); break;\
		case 16: \
			kernelV5Atmoic64(16,gridDim, blockDim, sharedLimit); break;\
		case 32: \
			kernelV5Atmoic64(32,gridDim, blockDim, sharedLimit); break;\
		case 64: \
			kernelV5Atmoic64(64,gridDim, blockDim, sharedLimit); break;\
		case 128: \
			kernelV5Atmoic64(128,gridDim, blockDim, sharedLimit); break;\
		default: \
...
#define switchKernelV5Config(configs) \
	switchKernelV5(configs.atomic64,vwSize,gdim, bdim,sharedLimit)